...
class Minesweeper:
    ...
    def __init__(self, root):
        ...
        self.diffs = {
            "50/50":  {"c": 2,  "r": 1,  "m": 1},
            "Easy":   {"c": 7,  "r": 7,  "m": 8},
            "Medium":   {"c": 14, "r": 14, "m": 30},
            "Hard": {"c": 17, "r": 17, "m": 50},
            "Impossible": {"c": 1000, "r": 100, "m": 15000},
            "WTH":     {"c": 3,  "r": 3,  "m": 0},
        }
        self.cur = "Easy"
        self.S = 36
    ...
    def _ui(self):
        ...
        tk.Label(left, text="Difficulty:", bg=self.DKG,
        ...
        tk.Button(left, text="New", command=self.reset,
                  font=("Segoe UI", 9), bd=1, relief=tk.FLAT,
                  overrelief=tk.RAISED, bg="white", cursor="hand2"
                  ).pack(side=tk.LEFT, padx=2)

        self.hbtn = tk.Button(left, text="Help", command=self.help,
        ...
        tk.Label(bar, text="mines", font=("Segoe UI", 8),
                 fg="#81C784", bg=self.DKG).pack(side=tk.LEFT, padx=(2, 0))

        self.face = tk.Label(bar, text=":-)", font=("Segoe UI", 18),
                             bg=self.DKG, cursor="hand2", fg="#E0E0E0")
        self.face.pack(side=tk.LEFT, expand=True)
        self.face.bind("<Button-1>", lambda e: self.reset())

        tk.Label(bar, text="clock", font=("Segoe UI", 8),
    ...
    def reset(self):
        ...
        if self.cur == "Impossible":
            self.psst.pack(side=tk.LEFT, padx=2)
        else:
            self.psst.pack_forget()
        self._imp_clicks = 0

        sz = {"50/50": 80, "Easy": 38, "Medium": 30, "Hard": 26, "Impossible": 6, "WTH": 100}
        self.S = sz[self.cur]
        ...
        if self.cur == "WTH":
            ...
            self.mlbl.config(text="Turn: X")
            self.hlbl.config(text="Tic-tac-toe! Click to place X.")
    ...
    def _lc(self, evt):
        ...
        if self.cur == "WTH":
            ...
            self.mlbl.config(text="Thinking...")
            self.face.config(text="O_o")
            if winner == "X":
                self.over = True
                self.ttt_game_over = True
                self.ticking = False
                self.face.config(text="B-)")
                self.hlbl.config(text="You win!")
                self.mlbl.config(text="X wins")
                return
            elif winner == "draw":
                self.over = True
                self.ttt_game_over = True
                self.ticking = False
                self.face.config(text="-_-")
                self.hlbl.config(text="Draw!")
                self.mlbl.config(text="Draw")
                return
            self.ttt_turn = "O"
            gid = self._game_id
    ...
    WIN_MSGS = [
        "You survived.",
        "Touched grass today? No.",
        "The government fears your power.",
        "Certified minesweeper.",
    ]
    ...
    def _ttt_ai_go(self, gid):
        ...
        if winner == "O":
            ...
            self.ticking = False
            self.face.config(text="X_X")
            self.hlbl.config(text="You lost!")
            self.mlbl.config(text="O wins")
            return
        elif winner == "draw":
            self.over = True
            self.ttt_game_over = True
    ...
    def _psst(self):
        if self.cur == "WTH":
            return
        if self.cur != "Impossible" or self.over or self._animating:
            return
        if self.first:
            self._place(0, 0)
            self.first = False
    ...
    def help(self):
        if self.cur == "WTH":
            if self.over:
                self.hlbl.config(text="Game over! Press 'New' to restart.")
                return
            if self.ttt_turn == "O":
                self.hlbl.config(text="Wait for your turn...")
                return
            b = self.ttt_board
            best = None
            for r in range(3):
            ...
            if best:
                ...
                self.hlbl.config(text=f"Suggestion: play at ({best[0]+1},{best[1]+1})")
            else:
                self.hlbl.config(text="Board full!")
            return
        if self.cur == "50/50":
            self.hlbl.config(text="No logical solution, it's 50/50.")
            return
        ...
        if not suggestions:
            remaining = sum(1 for r in range(self.R) for c in range(self.C)
                            if not self.rev[r][c] and not self.flg[r][c])
            mines_left = self.M - self.fcnt
            if remaining == mines_left and remaining > 0:
                for r in range(self.R):
                    for c in range(self.C):
                        if not self.rev[r][c] and not self.flg[r][c]:
        ...
        if not suggestions:
            remaining = sum(1 for r in range(self.R) for c in range(self.C)
                            if not self.rev[r][c] and not self.flg[r][c])
            mines_left = self.M - self.fcnt
            density = mines_left / remaining if remaining > 0 else 0
                                ...
                                for dc2 in (-1, 0, 1):
                                    ...
                                    if 0 <= nr2 < self.R and 0 <= nc2 < self.C:
                                        ...
                                        if not self.rev[nr2][nc2] and not self.flg[nr2][nc2]:
                                            hidden += 1
                            if hidden > 0:
                                probs.append((n - bcnt) / hidden)
                    prob = sum(probs) / len(probs) if probs else density
        ...
        for t, pos in suggestions[:8]:
            if t == "click":
                ...
                txts.append(f"Flag ({pos[0]+1},{pos[1]+1})")
            else:
                txts.append(f"? Click ({pos[0]+1},{pos[1]+1})")
        self.hlbl.config(text="Suggestions: " + " | ".join(txts))
